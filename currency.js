...
(function(root) {
     ...
     */
    currency.addTier2Support = function() {
      for (var key in currency.CurrencyInfoTier2) {
        currency.CurrencyInfo[key] =
            currency.CurrencyInfoTier2[key];
      }
    };
     ...
     */
    currency.getGlobalCurrencyPattern = function(currencyCode) {
      var info = currency.CurrencyInfo[currencyCode];
      var patternNum = info[0];
      if (currencyCode == info[1]) {
        return currency.getCurrencyPattern_(patternNum, info[1]);
      }
      return currencyCode + ' ' +
          currency.getCurrencyPattern_(patternNum, info[1]);
    };
     ...
     */
    currency.getGlobalCurrencySign = function(currencyCode) {
      var info = currency.CurrencyInfo[currencyCode];
     ...
     */
    currency.getLocalCurrencyPattern = function(currencyCode) {
      var info = currency.CurrencyInfo[currencyCode];
      return currency.getCurrencyPattern_(info[0], info[1]);
    };
     ...
     */
    currency.getLocalCurrencySign = function(currencyCode) {
      return currency.CurrencyInfo[currencyCode][1];
    };
     ...
     */
    currency.getPortableCurrencyPattern = function(currencyCode) {
      var info = currency.CurrencyInfo[currencyCode];
      return currency.getCurrencyPattern_(info[0], info[2]);
    };
     ...
     */
    currency.getPortableCurrencySign = function(currencyCode) {
      return currency.CurrencyInfo[currencyCode][2];
    };
     ...
     */
    currency.isPrefixSignPosition = function(currencyCode) {
      return (currency.CurrencyInfo[currencyCode][0] &
              currency.POSITION_FLAG_) == 0;
    };
     ...
     * @private
     */
    currency.getCurrencyPattern_ = function(patternNum, sign) {
      var strParts = ['#,##0'];
      var precision = patternNum & currency.PRECISION_MASK_;
      if (precision > 0) {
        strParts.push('.');
      ...
      if ((patternNum & currency.POSITION_FLAG_) == 0) {
        strParts.unshift((patternNum & currency.SPACE_FLAG_) ?
                         "' " : "'");
        strParts.unshift(sign);
        strParts.unshift("'");
      } else {
        strParts.push((patternNum & currency.SPACE_FLAG_) ? " '" : "'",
                      sign, "'");
      }
     ...
     */
    currency.adjustPrecision = function(pattern, currencyCode) {
      var strParts = ['0'];
      var info = currency.CurrencyInfo[currencyCode];
      var precision = info[0] & currency.PRECISION_MASK_;
      if (precision > 0) {
        strParts.push('.');
        for (var i = 0; i < precision; i++) {
          strParts.push('0');
